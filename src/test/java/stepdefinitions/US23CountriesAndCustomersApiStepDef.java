...
public class US23CountriesAndCustomersApiStepDef {
    ...
    //    @US023TC02
    @Given("user provites the api ent point to set the response using customers {string}")
    public void user_provites_the_api_ent_point_to_set_the_response_using_customers(String customer_api_url) {
        response = given().headers(
                "Authorization",
                "Bearer " + ConfigurationReader.getProperty("Selim_token"),
                "Content-Type",
    ...
    @And("creates customer using {string}and {string} and {string}")
    public void createsCustomerUsingAndAnd(String id, String firstName, String LastName) {
