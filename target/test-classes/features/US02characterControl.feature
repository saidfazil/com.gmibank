...
Feature:US_002 SSN, Mobil Phone, Email Charecter Control test
  ...
  @US02TC02
  Scenario: US02TC02 user must enter invalid data into SSN TextBox using "-" character.

    And new click on SSN TextBox
    And the user must type the character "-" when entering a number into the current SSN TextBox.
    Then ssn the user should see the text "Your SSN is invalid"
  ...
  Scenario: US02TC03 user must enter valid data in Mobile Phone Number TextBox using "-" character.
    ...
    And user must type the character "-" when entering a number into the invalid Mobile Phone Number TextBox
    Then mobil the user should see the text "Your mobile phone number is invalid".
  ...
  Scenario: US02TC04 user must enter valid data in Email TextBox using "@" character and ".com" expression
    ...
    And the user is entering a invalid Email TextBox. Email Must use " @ " character and ".com" expression in TextBox
    Then email the user should see the text "This field is invalid".